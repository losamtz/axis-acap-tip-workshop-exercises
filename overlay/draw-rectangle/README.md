...
This exercise draws a static rectangle over the video using `axoverlay` with the Cairo image backend.

`app/draw_rectangle.c` keeps the helper functions, callback implementations, signal handler, and Cairo drawing code in place. Complete the TODOs in `main()` with the snippets below.

The important callback flow is:

- `adjustment_cb` receives stream size and rotation changes and sets the overlay size.
- `render_overlay_cb` receives the Cairo context and calls the rectangle drawing helper.
- `axoverlay_redraw()` asks axoverlay to invoke the render callback.
...
This gives the app access to the graphics and overlay D-Bus APIs used by `axoverlay`.

## Step 3: Create the main loop and check the backend

Open `app/draw_rectangle.c`.

Paste this where the file says `TODO 1`:

```c
setenv("XDG_CACHE_HOME", "/usr/local/packages/draw_rectangle/localdata", 1);
...
This prepares the GLib loop and verifies that the device supports Cairo-backed overlays.
...
This connects axoverlay to the adjustment and render callbacks already implemented in the file.

## Step 5: Configure palette colors
...
The render callback maps these palette indexes to Cairo RGBA values before drawing.
...
The overlay is created at full camera resolution; `adjustment_cb` still handles rotation and stream changes.
...
This creates a palette overlay that the render callback clears and redraws with Cairo.
...
`axoverlay_redraw()` triggers the render callback once, then the main loop keeps the overlay active until the app stops.
...
This removes the overlay and releases axoverlay and GLib resources.
