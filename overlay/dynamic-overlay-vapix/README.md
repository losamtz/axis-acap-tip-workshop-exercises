...
This exercise is based on `overlay/dynamic-overlay-vapix` from the complete `axis-acap-tip-workshop` repository.

`app/dynamic_overview_vapix.c` keeps the original headers, helper functions, callbacks, signal handling, and other support code. Complete only the TODOs in `main()` by pasting the snippets below in order.
...
Complete source: `overlay/dynamic-overlay-vapix` in `axis-acap-tip-workshop`.
