...
This exercise is based on `overlay2/add-logo` from the complete `axis-acap-tip-workshop` repository.

`app/overlay2_add_logo.c` keeps the original headers, helper functions, callbacks, signal handling, and other support code. Complete only the TODOs in `main()` by pasting the snippets below in order.
