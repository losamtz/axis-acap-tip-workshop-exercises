...
This exercise is based on `overlay2/draw-views` from the complete `axis-acap-tip-workshop` repository.

`app/overlay2_draw_views.c` keeps the original headers, helper functions, callbacks, signal handling, and other support code. Complete only the TODOs in `main()` by pasting the snippets below in order.
