...
This exercise is based on `event/send-events-types/send-state` from the complete `axis-acap-tip-workshop` repository.

`app/send_state.c` keeps the original headers, helper functions, callbacks, signal handling, and other support code. Complete only the TODOs in `main()` by pasting the snippets below in order.
...
Complete source: `event/send-events-types/send-state` in `axis-acap-tip-workshop`.
