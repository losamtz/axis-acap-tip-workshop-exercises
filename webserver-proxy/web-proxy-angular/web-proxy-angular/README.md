...
This exercise is based on `webserver-proxy/web-proxy-angular/web-proxy-angular` from the complete `axis-acap-tip-workshop` repository.

`app/web_proxy_angular.c` keeps the original headers, helper functions, callbacks, signal handling, and other support code. Complete only the TODOs in `main()` by pasting the snippets below in order.
